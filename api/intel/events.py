...
from server.intel_store import event_cursor_key, export_user_data, set_seen_cursor
...
def handle(method: str, headers: dict, query: dict, body: bytes = b"{}"):
    def run():
        ...
        if method == "GET":
            ...
            cursors = user.get("last_seen_event_id_by_team") or {}
            cursor = (
                cursors.get(event_cursor_key(query["league_id"], query["team_id"]))
                or cursors.get(query["team_id"])  # legacy cursor written before league qualification
            )
        ...
        if method == "POST":
            ...
            set_seen_cursor(
                get_kv(), user_id, payload["team_id"], payload["event_id"],
