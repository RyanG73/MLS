...
- **Automated alerting** — `legacy/scripts/notify.py::notify_drift_alert(brier_recent, brier_baseline, pct_change)` already exists and is wired to ntfy.sh, but nothing calls it today (confirmed 2026-07-10: zero callers; archived to legacy/ in the 2026-07-19 dead-code audit, then deleted with that tree on 2026-08-04 — recover it with `git show e9043ed:legacy/scripts/notify.py` when wiring alerts). Wiring `build_drift_report.py`'s `alerts` list (churn leagues over threshold) to a notify call is the natural next step — the signature doesn't quite match (it's Brier-framed, this is odds-churn-framed), so either adapt the call or add a small `notify_churn_alert()` sibling function rather than force-fitting the existing one.
