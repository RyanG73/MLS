...
| Club Watch locks on club pages | 🟡 Built, not deployed | The club page had no paid surface at all despite being the highest-intent slot on the site. Three locked cards now sit at the foot of the profile grid, each pairing a real free number (30-day move from `movers.js`, next fixture and odds from the payload) with a locked continuity layer. Blurred content is prose, never a figure — a blurred number is still that number in the DOM. Card 2 gives away the win/draw/loss branch and names Next 5 Sim, because that tool computes it free on the same page; only being *told* is locked. Five tests lock the honesty rules. |
| Club Watch teaser rewritten | 🟡 Built, not deployed | The locked page sold the product as ten internal codenames; `Scenario studio` existed in exactly one place in the repo — that chip — despite `run_scenario()` being fully implemented server-side. Chip list replaced by three worked examples with real tables; every heading is now plain language; the scenario feature is anchored to Next 5 Sim, which readers already use free on league tables. `demoClub()` stars a real club — the visitor's own if they have starred one anywhere, else a recognisable fallback (the old path opened with "Acassuso"). Also rebuilt 1,108 stale `team_intelligence` artifacts that the ELO recalibration had invalidated. |
| Global power rankings recalibrated | 🟡 Built, not deployed | Two structural causes found. Ten UEFA top flights had no country coefficient, and an absent coefficient priors at `0.0` — Premier League strength on an EPL-anchored scale — which the bridge's ridge faithfully held. And `league_bridge` built its prior from `league_offset()`, which returns the fitted file, so each fit was ridged toward its own previous output and no coefficient edit could move it. Added the ten coefficients, a `_UEFA_UNLISTED_COEFF` floor, a `static_league_offset()` that never reads the fitted file, and a decaying `suspended_estimate` for Russia (no UEFA match since 2022). Refit adopted: held-out Brier 0.6139 → 0.6131, 10/10 seeds. Bodo/Glimt 4th → 39th, Zenit 5th → 57th, KuPS 9th → 99th; top ten now Bayern/Arsenal/Barcelona/Man City/Inter/Real Madrid/PSV/PSG/Man United/Club Brugge. Four regression tests added. Added coefficients are estimates pending a uefa.com refresh. |
| League + matches presentation pass | 🟡 Built, not deployed | League header no longer wraps "Division 1" and drops the build timestamp; race cards list only clubs with the definition as a footnote (derived from zone lines for the 70 leagues shipping no hint); Matches-page rails stack club names, label the swing score and explain it; matches slate loses the win percentages duplicated inside each probability bar and slims it to a 7px meter. |
| One colour palette for projection categories | 🟡 Built, not deployed | Three category-colour maps had drifted — MLS Cup was gold in the ladder, blue in the club-page trajectory chart, grey in the race-movement sparkline. All three now read one `METRIC_RGB` palette banded by meaning (gold win-outright, green qualify, blue secondary route, red danger), so a category is the same colour everywhere. Sparkline axes are grey in every category; season progress reads as a lighter grey against the remainder. Verified per-metric against the table's own heat colours on MLS, Brazil, Liga MX and EPL. |
| One ladder for every league page | 🟡 Built, not deployed | MLS was the last page on the pre-redesign half-width standings markup — no GP/GD columns, a 10px grey `17 GP · +21 GD · +2.8 xGD` sub-line under each club, 12px names. `tableLadder()` and `ladder()` now both delegate to a single `ladderPanel()`, so grid, type scale, club cell, cut lines and the Club Watch overlay are written once. MLS keeps two conferences, five outcome columns and its host/wild-card/playoff cuts in the shared chrome, stacked full width; its race strip uses the shared `raceCard`. All 78 live leagues swept and rendering — 70 table, 7 knockout, 1 MLS, zero failures, zero legacy markup. 1,648 tests pass with 14 skips. |
| Desktop home round 3 | 🟡 Built, not deployed | One league now drives the whole upper page: the rotation's league feeds the table, the Title Probabilities board (Belgium added, plus a reader-controlled slot row defaulting to the Netherlands), two new side-by-side vertical Upcoming/Recent columns carrying club names, and Biggest Movers. A dropdown of the 62 off-rotation leagues lazily loads that league and pins every panel to it. Masthead lost the build timestamp; `78 leagues live` moved left so search stands alone. `build_movers.py` withholds saturated-baseline movers — 46 suppressed, 0 of the 0↔100 artifacts remain. 1,648 tests pass with 14 skips; verified at 1440px and 375px with no console errors. |
...
| Race "Changes since" diff mode | 🟡 Built, not deployed | The race panel renders as a diff against a reader-chosen baseline date, modelled on the FanGraphs playoff-odds Changes mode. URL-addressable (`?league=mls&mode=changes&since=2026-03-01`), works logged out, no personalization — deliberately not the personalized "since your last visit" digest, which stays a separate paid feature. Needed no new pipeline: `DRIFT_TRAJ` already ships per-point `kind` provenance. Every diff carries the provenance of **both** endpoints; a window straddling the reconstruction boundary says so in words, not only in dashed-vs-solid styling, and flags each affected row `replay`. Baselines never interpolate — a date the model did not publish on resolves to the last one it did. Verified: EPL 7-day window reports archived; MLS `since=2026-03-01` reports mixed with 12/12 rows flagged; Bundesliga 2026-07-30→31 renders "Nothing moved."; round-trip clears both URL params. Seven contract tests; no console errors at 375px or 1280px. |
| Crossbar — the named strength scale | ✅ Live | `https://entenser.com/crossbar/` returns 200 and is in the live sitemap. The cross-league scale is publicly named **Crossbar** (owner decision 2026-08-01; payload field stays `global_elo`). Free self-canonical page with `DefinedTerm` JSON-LD, linked from every club page that shows the number — the live Arsenal page reads `Crossbar 1756` and links through. Figures are read from the payloads at build time, not written down: 1,172 clubs, 71 competitions, 695–1,770. States what the scale is *not* (form table, bookmaker-derived, betting rating, trophy count), that the league offset cannot change a forecast, and the evidence tier behind each league's placement (41 fitted, 13 confederation-anchored, 9 estimated priors, 7 tier bridges, 1 frozen). Seven contract tests, one asserting no accuracy or betting claim. |
