...
| Global Power and Crossbar | ✅ Live | 1,167 clubs across 71 competitions carry a Crossbar rating (measured 2026-08-05). The bridged Global Power ladder at `?league=power` is a strictly smaller population — 965 clubs across 55 leagues, because it additionally requires measured bridge evidence. Two correct numbers for two different questions; both are registered in `docs/figures.json`. Publicly named **Crossbar**; `global_elo` remains the payload field. Displayed on club pages, league tables, projection context, team pages, selectors, run-in difficulty and history charts, and ranked at `?league=power`. |
