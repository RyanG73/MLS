...
| Club Watch season forecast history | 🟡 Site/API deployed; record publication pending | Club Watch now merges the current-season point-in-time replays with the exact nightly archive, labels dashed reconstructed versus solid archived checkpoints, lets members inspect available historical targets, and includes a frozen history chart in the complete free sample. Exact archives win same-day conflicts; public league history remains free. Pages run `30643779327` and API run `30643779410` deployed commit `2e5f14a`. The refresh rebuilt 1,108 team records but exposed missing publisher credentials. API run `30645130425` then proved the rebuild clean but confirmed Vercel sensitive values are intentionally non-exportable. Publication now uses a scoped authenticated API relay so Upstash credentials stay inside Vercel; final production proof is pending. |
