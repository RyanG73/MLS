...
| Platform reliability — Phase 1 of the 2026-08-09 spec | 🔄 Merged as code and CI config; **no scheduled run has exercised it yet** | Branch `auto/spec-exec-2026-08-09`. **MLS schedule routed** — `build_dashboard_data` resolves its schedule through `source_registry` with ESPN first and API-Football (af_id 253) as ordered fallback, so a 403 on `usa.1/scoreboard` stops being fatal. ESPN stays primary deliberately: a healthy build takes the rows it takes today. **The fallback is guarded and currently fails closed** — `config/api_football_team_names.json` has no `mls` entry, and below 95% of club names resolving to an ASA id the spine answer is refused rather than shipping a short season. Generating that map needs a live diff (§3.2's pattern) and is the remaining work before `build (mls)` is actually green with ESPN dark. **Rebuild serialization**: `refresh-leagues.yml` carries a constant concurrency group, `cancel-in-progress: false`. **Source health**: `scripts/build_source_health.py` folds each run's gitignored parquet into a committed JSON summary and fails the run after three consecutive dark runs for a feed; `--check` runs after the commit so a verdict cannot discard good payloads. **Fault-injection drill**: weekly, `ENTENSER_BLOCK_ESPN=1`, asserting a routed league survives, an unrouted one fails visibly, and the MLS schedule falls through. Verified by test, not by a scheduled run. |
| Data-integrity checks — standings and payload self-consistency | 🔄 Merged; standings check is credential-blocked until it runs in CI | `scripts/check_standings.py` diffs every published table against API-Football `/standings` (~78 requests/day, exact name matching only — a fuzzy matcher proposed `Bristol City → Stoke City` during Stage 3). Verified offline against the committed payloads: with no credential every league reports `error` with its reason and the exit code stays 0. `scripts/check_payload_consistency.py` reads the payloads the way a client does and checks club sets, each club's `gp` against its own fixtures, pair multiplicity, and names differing only by whitespace; `config/payload_consistency_baseline.json` records today's measured deviations so a NEW one fails the run. **Two live findings, neither adjudicated:** NWSL publishes `gp` one below the fixture count for Gotham FC and Kansas City Current (the two clubs meet twice), and `australia-aleague-women` carries a club named `"Western Sydney "` with a trailing space — the §8.3 shape that was 2,114 rows from deleting a third of a league. |
| `check_docs` (§10.7) | ⚠️ **Failing on `main`, and was before this work** | 14 figure problems, of three kinds. **Genuinely stale on a live surface (2):** this file's Global ELO row and the active plan's verification line both still read `697–1,797`, which `docs/figures.json` supersedes. **False positives (2):** the stale-value grep is `\b<value>\b`, so `748` matches inside `0.748` in `postgame-win-expectancy.md` and `750` inside `~1,750 tests` in the active plan. **Historical prose (10):** `research-log.md`, `PROJECT_HISTORY.md` and a dated competitor teardown naming values the record needs. Not repaired here — outside the 2026-08-09 roadmap, and the first group is a fact correction that needs the repo-wide `rg` sweep `CLAUDE.md` requires. |
