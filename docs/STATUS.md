...
**Confirmed 2026-08-15 by the owner, in these words: "confirming that we are good to
focus on free."** This settles `A3` — the objective below is no longer a proposal
awaiting review, and no session should re-open it. There is no transaction to
complete while the product is free, so the milestone below replaces the one this
section carried from 2026-07-27. The old one is not cancelled — it is deferred, and
it returns intact with the paywall.
...
| Surface | State | Proof |
|---|---|---|
| **Fuzzy fixture matching was still live where it writes scorelines** | ✅ **Fixed 2026-08-15** | `check_standings` banned fuzzy matching outright in Stage 3 — a fuzzy matcher there proposed `Bristol City → Stoke City`. **The same technique survived in `fast_refresh`, which runs every fifteen minutes and writes results into the table.** It scored a fixture as the *average* of two `SequenceMatcher` ratios with a **0.58** floor, then took `max()` with **no ambiguity guard at all**. **Measured against real confusables at that rule: 13 of 15 genuinely different clubs cleared it** — Manchester United/Manchester City **0.812**, Deportivo Cali/Deportivo Pasto 0.759, Atletico Nacional/Atletico Junior 0.750, Newcastle United/Newcastle Jets 0.733, Queens Park/Queens Park Rangers 0.733, and Bristol City/Stoke City itself at **0.727**. Three separate defects: **(a)** averaging let a perfect home match carry a hopeless away one, when a fixture is identified by both clubs or neither — it now scores as its **weaker** side; **(b)** the floor was far too low — matching is now token-exact on both clubs first, with similarity accepted only at **0.90 on each side**; **(c)** there was no ambiguity guard, so on a derby round `max()` silently picked one — a runner-up within **0.10** now refuses, as does a second token-exact candidate. **Refusing is the safe direction**: `apply_feed` skips an unmatched row and the next tick retries, so the cost is one cycle. Guessing costs a result attributed to the wrong club, propagated into the table, the simulation and the published forecast, with nothing downstream able to tell. **And refusing is no longer silent** — an unmatched *completed* row is reported, because a league whose feed names drifted would otherwise just stop updating and still look healthy. Deliberately in the run report, not in `payload["fast_refresh"]`, which ships to the client. Validated both directions: 8 of 8 confusables refused, and real spelling variation (`Arsenal`/`Arsenal FC`, `Inter Miami CF`/`Inter Miami`, `Peñarol`/`Penarol`) still matches. 17 new tests; 2,201 pass. |
| **The spine catalogue no longer hardcodes six leagues** | ✅ **Shipped 2026-08-15 — and it routes nothing by itself** | `api_football.LEAGUE` carried six hand-written `(af_id, seasons)` tuples, so a league could be approved, mapped and name-proven and still be unreachable. `league_spec(league_id)` now falls back to the approved 78-league catalogue (`config/api_football_league_map.json`) for any league without a hand entry. **The hand entries still win outright**, which is the point: they encode measured decisions the map cannot express — K League 1 excludes 2016/2017 because 2017 is a 15-fixture data hole, and a derived range would silently re-introduce it. Derived history is floored at `HISTORY_FLOOR = 2017`, matching the training-data rule, because every season below it is real paid requests for data no model may train on. ⚠️ **Read this before assuming leagues have moved: reachable is not routed.** A league is built from the spine only when `source_registry.REGISTRY` names it, and REGISTRY is unchanged — still `northern-super-league`, `usl-super-league`, and MLS as *fallback* with ESPN primary. A test asserts the property directly (`epl` is in the catalogue and still resolves to `["espn"]`). 5 tests added; suite `2215 passed, 36 skipped`. |
| **Routing the 18 validated leagues — what is actually still in the way** | ⛔ **Open — gated on evidence, not on effort** | Worth stating plainly because "18 of 78 passed" reads like 18 leagues are ready to flip, and they are not. The name map is **step 3 of the four-step gate** in the reliability spec §3.2, not the whole gate. `derive_pairs` proves *name pairs* — that a spine club and one of ours play the same matches, on the same dates, with the same scorelines, at least three times and contradicting nowhere. It does **not** compute a full-history scoreline-agreement rate, and it does not compare standings. §3.2 requires both: *"must reach 100% scoreline agreement and identical standings before anything moves."* The awkward part is step 2's comparison arm — the gate diffs against the ESPN frame, and **ESPN was dark** (see the one-source row below), so a live diff was not possible. **That obstacle is gone as of 2026-08-23** — ESPN was dark because of our own User-Agent, and it answers again; a live full-history diff is now available, so the substitute-the-payloads judgement the spec never made does not have to be made. The rest of the gate is unchanged: agreement rate and standings still have to be computed and cleared before anything routes. The committed payloads are ESPN-derived and are the obvious substitute, but substituting them is a judgement the spec has not made. **Next action:** extend the derivation to report per-league agreement rate and a standings comparison off the payload frame it already loads, then flip only the leagues that clear it, smallest first, with the observation week §3.2 asks for. Nothing should be routed before that. |
| **`Spine Name Maps` reported success while producing nothing** | ✅ **Fixed 2026-08-15** | Two defects, found by actually reading run [`31918862701`](https://github.com/RyanG73/MLS/actions/runs/31918862701) rather than its status badge. **(1)** `tee output/name-map-report.txt` died on its first line — `No such file or directory`, because nothing creates `output/` in a fresh checkout — so the report was never written and `gh run download` returned "no valid artifacts found". The 18-of-78 result survived only in the run log. **(2)** The step exited 1 and still rendered ✅, because `continue-on-error: true` was carrying two meanings at once: a league on HOLD (intended) and a genuinely broken step (not intended). Both exited 1, so neither the job status nor the exit code could tell them apart. Fixed by `mkdir -p output` plus `set -o pipefail`; by giving HOLD its own exit code (`EXIT_HELD = 3`) so `continue-on-error` could be **removed** and any other non-zero code now fails the job; and by moving the artifact upload from `if-no-files-found: warn` to `error`, since `warn` is what let the empty artifact pass unremarked. 3 new tests (14 in the file, up from 11) pin the contract: HOLD exits 3 and not 1, all-clear exits 0, and `--check` never writes the map. Full suite `2210 passed, 36 skipped`. **Not yet re-run in CI** — the next dispatch is what proves the artifact lands. |
| **Getting off ESPN — the name-map blocker** | 🔄 **Derivation has now run: 18 of 78 leagues are routable, 2026-08-15** | **ESPN was never removed, and the record should say so plainly.** `source_registry` routes exactly **3 of 78** leagues, and MLS is routed ESPN-*first* by design — so ESPN is still the primary fixtures source for ~75 leagues, which is why 49 payloads went stale when it went dark. `espn_continental` is also the evidence base behind Global ELO (`league_bridge`, `club_bridge`, `interconf_calibrate`), so removing ESPN is not one change. **The blocker was never routing — it was club-name maps.** `config/api_football_team_names.json` covers 3 leagues because MLS's was worked out by hand. An unmapped club is *not* a visibly missing club: it fails to resolve, is discarded downstream as "not in this league", and the payload still looks healthy, short a team nobody counted. `scripts/generate_spine_name_maps.py` generalises the protocol that proved MLS: **a pair is proven by fixture diff — date, opponent and scoreline — never by name similarity**, because fuzzy matching once proposed `Bristol City → Stoke City`, which would have attributed one club's season to another and looked entirely plausible. Seeded constraint propagation from production's own token matcher; a club whose fixtures corroborate two targets equally is **refused, not guessed**; the floor is *every* club, not a share, because 29 of 30 clears any percentage and silently drops a club. 11 tests, including the indistinguishable-fixtures case and the one-day kickoff slack. ⚠️ **It cannot run from a developer checkout** — `API_FOOTBALL_KEY` lives only in CI — so `.github/workflows/spine-name-maps.yml` runs it, report-only by default. **Remaining after the maps land:** wire `api_football.LEAGUE` from the 78-league map (it currently hardcodes 6), flip the registry per validated league, then move the continental evidence base. Routing a league before its map is proven is the one thing that must not happen. **First derivation run, 2026-08-15 — report-only, `write: false`, `Commit the map` skipped: [`31918862701`](https://github.com/RyanG73/MLS/actions/runs/31918862701).** The script's own summary line reads `18/78 leagues cleared the all-clubs floor`. Clearing today: `argentina-primera` `australia-aleague-women` `brazil-serie-a` `canadian-pl` `china-super` `finland-veikkausliiga` `india-isl` `ireland-premier` `k-league-1` `mls` `northern-super-league` `norway-eliteserien` `nwsl` `romania-liga1` `sweden-allsvenskan` `swiss-super-league` `uruguay-primera` `usl-championship`. **The 60 holds split two ways, and the larger group is a calendar problem rather than a data one:** 28 produced no candidate pairs at all — `epl`, `la-liga`, `bundesliga`, `ligue-1`, `championship`, `eredivisie`, `belgian-pro`, `australia-aleague` among them, i.e. the winter-calendar leagues that are one or two matchdays into 2026-27 and have no played fixtures to prove a pair against; the other 32 are near-misses that found real pairs but fell under `MIN_CORROBORATING_FIXTURES = 3` (per-club corroboration counts cluster at 0/1/2 — `austria-bundesliga` shows four clubs at `only 2 corroborating fixture(s), need 3`). Those should convert on their own as matchdays accumulate; that expectation is an inference from the evidence pattern and has **not** been confirmed by a second run. ⚠️ **`usl-super-league` is routed in production today and the check now HOLDs it at 0/8 clubs** — plausibly off-season rather than a broken map, but it is one of only three live routes and is unverified either way. |
...
| **The data pipeline is running on one source** | 🔄 **Root-caused and fixed in code 2026-08-23; proof is the next scheduled run** | ESPN is dark across the board and nothing in this table said so. Measured from `data/source_health_summary.json` as written by daily run `31797104344` (2026-08-14 12:01 UTC): **`espn_fixtures` 384/384 fetches failed, `espn` 28/29 failed, `espn_continental` 83/83 failed and is dark for 6 consecutive runs, `football_data` 41/41 failed.** Only `api_football` answered — 70 of 71. The daily refresh therefore **fails at its own gate**: `build_source_health.py --check` exits 1 with "failing the run — dark feeds: espn_continental (6 runs)". That gate is working as designed and runs *after* the commit, so good payloads still land; the run is red because the sources are, not because the guard is wrong. **User-visible consequence, measured from the committed payloads: 49 of 91 carry a `generated` stamp of 2026-08-03/04/05** — `epl`, `la-liga`, `bundesliga`, `championship`, `eredivisie`, `belgian-pro` among them — against 33 stamped 2026-08-14. The European top flights have not had a real data rebuild in 12 days, with the 2026-27 season opening. This is the single largest correctness risk in front of the launch and it is Claude-executable: the fix is to route more of Tier A onto the spine that is still answering. **Root cause found 2026-08-23, and it was not a rate limit: it was our User-Agent.** `data_pipeline/http.py` sent `Mozilla/5.0`. ESPN now admits a request on the FIRST token of its User-Agent — a recognised HTTP client is served, a browser string is refused — so every ESPN call in the pipeline 403'd, the retry ladder spent four requests on each, and the breaker opened after five leagues and failed the rest without a request. Measured 2026-08-23, one IP, five trials per agent run in both orders: `curl/8.7.1`, `python-requests/2.32.3`, `python-requests/9.9.9`, `okhttp/4.9.3` and `Go-http-client/1.1` all **200**; `Mozilla/5.0`, a full Chrome 126 string, `Entenser/1.0 (+https://entenser.com)` and an empty agent all **403**. The split is by agent alone and is deterministic — a cold IP's first request 403s, which no rate limit explains. The agent is now built from `requests.utils.default_user_agent()` so it cannot drift from the client making the call, with `(+https://entenser.com)` appended (free at the filter, and it says who is calling). Two scripts that bypass `espn_get` — `fetch_league_teams.py` and `fetch_foreign_logos.py` — carried the same browser string and are fixed through one shared `espn_user_agent()` helper; `fetch_foreign_logos` uses urllib and therefore leads with `Python-urllib/…`, because leading with a token that does not match the library is the same lie in a new costume. **Verified live 2026-08-23 from this environment:** all 7 leagues `select_leagues()` returns now fetch (`refreshed=7 failed=0`, computed workflow exit code **0**, against 14 of 14 failing in run `32642980914`), and `uefa.champions`, `concacaf.leagues.cup`, `usa.1` and `eng.2/teams` — dark for 83/83, 28/29 and 384/384 fetches on 2026-08-14 — all answer. 2,217 tests pass. **Proven on a runner 2026-08-23**: dispatch `32644398934` on commit `aec36847` reported `refreshed: 66, failed: 0` — MLS applied 11 results, the Championship 9, J1 League 10 — against 14 of 14 failing in `32642980914` an hour earlier. GitHub's egress carries no additional block. The ESPN half of this row is closed. |
...
| Fast result/projection refresh | ✅ **Green and doing real work — run `32644964274`, 2026-08-23 14:19 UTC** | **This row read "✅ Live" on the strength of run `30205921705` from 2026-07-26 and was false when re-checked.** See the ⛔ row at the top of this table for the measured failure rate, the three compounding causes, and the fix. Kept here rather than deleted because a row that was wrong for nineteen days is itself the finding: the proof was a run ID that had stopped being re-run, which is how a green stamp outlives the thing it certified. Any row whose proof is a single dated run needs re-running, not re-reading. **Owner-reported 2026-08-23: red on every scheduled run.** It has failed continuously since run `32428963834` (2026-08-20 23:30 UTC), the last green one — **every** run in the 100 the API returns after it. The 2026-08-15 fix is not what is failing: the job is doing exactly what it was built to do. All 14 selected leagues 403'd at ESPN, `_FAILURE_TOLERANCE` is 0.5, so `fast_refresh.py --refresh-selected` exits 1 and the alert fires — a correct red on a real outage, not a broken guard. The cause was the User-Agent in the one-source row above. **Two bugs had to be cleared, and the second was hidden behind the first.** With ESPN answering, the run got three steps further and died in `validate_payloads`: `canadian-pl` and `k-league-1` reported `data_status 'historical'` against a registry saying `full_forecast`. Those payloads are correct on `main` — `validate_payloads.py` reports *All 79 payload(s) valid* against pristine `origin/main`. The mismatch was manufactured inside the run by the **Overlay prior live-data snapshot** step, which replaces `webapp/data` from the `live-data` branch but leaves `webapp/leagues.js` on `main`. `live-data`'s tip was `0da90a92` from **2026-08-04**, 19 days frozen, and predated the promotion of both leagues. **It could not self-heal**: validation runs before the publish step, so the failing run was the very run that would have refreshed the branch, and neither league is ever in the fast-refresh selection. Reset to `main`'s `webapp/data` (`695fd0ae`, rollback `0da90a92`). **Verified:** `32644964274` selected 7 leagues, `refreshed: 7 failed: 0`, **30 results applied** across EPL, La Liga, Ligue 1, Liga MX, Eerste Divisie and Segunda, *All 79 payload(s) valid*, and the job republished `live-data` itself at 14:19 UTC (`f224f835`) — the branch is self-sustaining again. **Note on the green ticks before this:** the prior 'success', `32428963834`, logged `Fast-refresh leagues: []` and skipped every step. This job has done no real work since roughly 2026-08-04; a green tick on a conditional job certifies nothing. **Correction, 2026-08-23: the frozen branch was NOT the user-facing rollback this row claimed**, and the correction is worth more than the note it replaces. `webapp/index.html` does overlay the current league's payload from `live-data`, but it called `_overlayLiveData` from the same inline block that `document.write`s the same-origin payload — and a written `<script>` executes only when the parser resumes, i.e. after that whole block ends. The deployed file therefore overwrote the overlay every time, so **the overlay had never once applied** and no reader was ever rolled back. Measured on entenser.com: `?league=epl` rendered `window.LEAGUE_DATA.generated` = `2026-08-03 06:53 UTC`, the deployed stamp, while `live-data` carried `2026-08-23 14:17 UTC`. The real cost was the mirror image of the one recorded here — not stale data pushed over fresh, but fresh data that never arrived: 20-day-old EPL, La Liga and Ligue 1 projections on a site whose overlay existed to refresh them every 15 minutes. ✅ **Both overlays now guard on `generated`, and the browser one now runs where it can see one.** One rule, stated the same way in both places: a `live-data` payload is used only when its stamp is provably newer than the one it would replace; an unreadable stamp loses, so an overlay can only ever move a payload FORWARD in time. CI extracts the branch to `$RUNNER_TEMP` and reconciles it per file through `scripts/overlay_live_data.py` instead of `tar -x`-ing it over the checkout — chosen over also overlaying `webapp/leagues.js` from the branch, because that inverts which tree is authoritative about the league set and would still ship stale numbers. The browser call sites defer through `_writeOverlay`, which writes the call into the document stream behind the payload script; the guard is what makes running it there safe, since its worst case is exactly the behaviour shipping today. **Proof, A/B over a reconstructed frozen branch** (the live one at `f224f835` with `canadian-pl` and `k-league-1` put back to `historical` at 2026-08-04 13:05 UTC, against pristine `origin/main` `bbe3ab47`): the blanket `tar -x` reproduces the outage verbatim — `validate_payloads` exits 1 with *2/79 payload(s) failed validation* and the same two `data_status mismatch` lines quoted above; the guarded step on the same input prints *keep canadian-pl.js … (live-data is older)* and *All 79 payload(s) valid*, exit 0. It is not simply refusing to overlay: it still takes **9 of 94** payloads — exactly the six leagues this run applied results to, plus `home.js`, `calendar.js` and `match-leverage.js`. `tests/test_live_data_overlay.py` (33 tests) pins all three halves independently: reverting `refresh-fast.yml` fails 2, reverting `webapp/index.html` fails 4, and neutering the comparison fails 9. The browser guard is driven in a real JS engine over the shipped source, and its take / keep / fail-closed behaviour was confirmed in a browser before shipping. |
...
| Data-integrity checks — standings and payload self-consistency | 🔄 Merged; standings check is credential-blocked until it runs in CI | `scripts/check_standings.py` diffs every published table against API-Football `/standings` (~78 requests/day, exact name matching only — a fuzzy matcher proposed `Bristol City → Stoke City` during Stage 3). Verified offline against the committed payloads: with no credential every league reports `error` with its reason and the exit code stays 0. `scripts/check_payload_consistency.py` reads the payloads the way a client does and checks club sets, each club's `gp` against its own fixtures, pair multiplicity, and names differing only by whitespace; `config/payload_consistency_baseline.json` records today's measured deviations so a NEW one fails the run. **Two findings adjudicated 2026-08-09, a third on 2026-08-23.** *NWSL `gp`*: the TABLE WAS RIGHT. Gotham and Kansas City each read one game short because the `games` list published their 2026-06-26 meeting (Gotham 2-0) while the standings loop correctly filtered it — ASA marks it `is_playoff=1`, the only knockout fixture in NWSL's 2026 season. Proved by arithmetic on both clubs at once: dropping that one fixture gives Gotham 18gp/37pts and Kansas City 18gp/29pts, which is exactly what the table publishes, while "drop each club's most recent match" gives Kansas City 28 and fails. The builder now emits `knockout: true` on such fixtures and both consistency checks skip them — deliberately NOT keyed `ko`, which is already the kickoff timestamp on upcoming cards in 32 leagues; reusing it read every scheduled fixture as a knockout and silently emptied the checks. *Trailing space*: six clubs, not one, all in `australia-aleague-women`; fixed at the ESPN source boundary in commit `e73049d0`. **2026-08-23 — `sweden-allsvenskan` pairs: the PAYLOAD WAS RIGHT and the CHECK WAS WRONG.** The payload publishes 16 clubs and 127 played matches, 111 pairs having met once and 8 having met twice: a 30-round double round-robin caught at round 16, the first round of its reverse fixtures. Nothing is missing or duplicated — `clubs`, `played` and `names` all pass, every club reads 15 or 16 games played, and the single pair still at zero is a postponement. The defect was the check's *uniformity* test, which asked every pair to have met the same number of times — true only at the two instants a season sits exactly between cycles. Git proves the mechanism: the payload rolled to the 2026 season on 08-05 and stood at `{1: 119}` on 08-09 when the baseline was measured, the one window in a Swedish season when uniformity could hold; the 08-17 refresh took it to `{1: 111, 2: 8}` and the check went red on a payload nobody had touched. Left alone it would recur for every double round-robin in the catalogue, each August–May league crossing the same line around the new year, until the baseline had swallowed the catalogue and the check meant nothing. `pairs` now asserts that no pair is two whole meetings AHEAD of another — the strongest claim pair counts alone support, since a round-robin of any depth puts every pair on either k or k+1 meetings. Argentina still fails on its real signature (1x, 2x and 3x at once, `spread` 2), and the baseline SHRANK rather than grew: `pairs` 30 → 18 deviating, twelve leagues released, **zero new**, verified by running the HEAD checker and the fixed checker over the same committed payloads — `clubs`, `played` and `names` came back byte-identical, so the change is isolated to `pairs`. The one case given up — a duplicate fixture inside a *complete* season, which counts cannot tell from a part-played 3x league — is held by `played`, which is where the NWSL finding above was caught in the first place. Regenerating also cleared four entries the baseline was carrying for leagues that had already resolved on their own (`played` 11 → 8, plus `argentina-nacional` on `pairs`); those were stale at HEAD, not affected by this change. Proof: `2216 passed, 39 skipped` and `check_payload_consistency.py --strict` exits 0 on pristine payloads. |
...
against the current objective: **a supporter completing the whole Club Watch job, free** (confirmed
by the owner 2026-08-15; this line said "one complete production transaction" until then and was
contradicting the objective section above). None of these need Stripe to be live, and every one of
them is work that objective depends on — nobody returns after a matchweek for a forecast built on
twelve-day-old data, and a rehearsal cannot be trusted against a red suite.
...
**The green scheduled run they owed arrived 2026-08-23** — `32646234681`, `refreshed: 8 failed: 0`,
selecting `northern-super-league` on the spine — after the ESPN User-Agent fix and the `live-data`
reset. That debt is paid.

**Re-ranked 2026-08-23. #8 is now the top of the queue, and #3 no longer says what it used to.**
The one-source row above records why: ESPN was never rate-limiting us, it was refusing our
User-Agent, and it has answered on every endpoint since. So the *premise* under #3 — "ESPN is
dark, route away from it" — is gone, while the *problem* under #3 is not: **51 of 90 payloads on
`origin/main` still carry a `generated` stamp older than 2026-08-20, 43 of them from
2026-08-03/04/05** (measured 2026-08-23 from the committed payloads). The job that rebuilds them
is the daily refresh, it has been red for four consecutive days, and with ESPN restored it has
exactly one named blocker left. That is #8, and it is cheaper and more certain than #3.
...
| 3 | **Move the stale Tier-A leagues onto the spine** — *step 1 of 3 shipped 2026-08-15* | **Premise corrected 2026-08-23 — read this before working the row.** The staleness is real and re-measured: **51 of 90 payloads** on `origin/main` predate 2026-08-20, 43 of them from 2026-08-03/04/05, including every European top flight with the 2026-27 season under way. What is no longer true is the reason given for it. This row said *"ESPN is dark (`espn_fixtures` 384/384 failed); `api_football` answers 70 of 71"* and concluded the remedy was to route away from ESPN. ESPN was dark because of our own `User-Agent`, it answers on every endpoint since 2026-08-23, and the 384/384 figure is a measurement of our bug, not of ESPN. **So this row is no longer the cheapest way to un-stale the payloads — #8 is**, because the daily refresh already rebuilds all 90 and needs one blocker cleared rather than 51 leagues individually gated. Routing to the spine remains worth doing for *resilience* — one source is still one source, which is what made a single header a site-wide outage — but it is now a redundancy argument, not an outage response, and it must still clear the full §3.2 gate. The plan is bought and paid for; CI already runs `API_FOOTBALL_PLAN: mega` | Ranked third only because it is per-league validated work, not because it matters least. Precedent exists: three leagues are already routed, and the MLS name map was proven by fixture diff rather than name similarity |
...
| ~~11~~ | ✅ **DONE 2026-08-23** — ~~Decide whether `refresh-daily.yml` should rebuild a league it believes is finished~~ | Answered yes, and the selection is now a tested module rather than an untestable heredoc. `scripts/select_daily_leagues.py` picks every live league **plus any claiming to be finished that has played within `MAX_IDLE_DAYS`** — the window imported from `scripts/eval/season_state.py`, so this filter and the builder's own CONCLUDED guard cannot drift apart | Measured against the committed payloads: **28 → 36** matrix jobs, and the eight added are exactly the eight leagues that were stuck. Nothing dropped. 9 tests, including the idle boundary and the `search-index.js`-is-an-array case. The detect job runs on bare `python3` with no `pip install`, so the script is stdlib-only and proven so under `python3 -S` |
...
|---:|---|---|---|
| ~~A1~~ | ✅ **DONE 2026-08-15** — ~~Delete two merged branches~~ | Owner ran the deletes. Both refs are gone from `origin`, verified with `git ls-remote --heads origin` returning nothing for either name | Closed. Nothing was lost: both were ancestors of `main` |
| ~~A2~~ | ✅ **DONE 2026-08-15** — ~~Say go on dispatching `Spine Name Maps`~~ | Owner said go; dispatched report-only as run [`31918862701`](https://github.com/RyanG73/MLS/actions/runs/31918862701). The `Commit the map` step shows `skipped`, so nothing was written | **Answered: 18 of 78 clear the all-clubs floor today**, against 3 currently routed. See the ESPN row above for the breakdown and for what the run exposed in the workflow itself |
| ~~A3~~ | ✅ **CONFIRMED 2026-08-15** — ~~Confirm or reject the rewritten objective~~ | Owner: *"confirming that we are good to focus on free."* The free-launch objective stands and is no longer provisional | Settled. The transaction milestone is deferred, not deleted, and returns with the paywall. Recorded as a durable decision in `PROJECT_HISTORY.md` and `CLAUDE.md` so it is not re-litigated |

**Tier A is clear as of 2026-08-15.** All three items are closed. What that unblocked is recorded
in the rows above — the name-map derivation ran, the catalogue was widened, and the objective is
settled.

**One Tier-A follow-on still needs an owner hand: the `write: true` dispatch.** It is not a new
ask, it is the second half of `A2` — the run that actually lands the 18 derived maps into
`config/api_football_team_names.json`. It could not be dispatched from the session that did the
rest, because the tooling blocks a dispatch that both spends budget and commits a file:

```bash
gh workflow run "Spine Name Maps" --ref main -f write=true
```

Safe to run as-is, for a reason worth knowing rather than trusting: `main()` seeds its output from
the *existing* file and only overwrites a league that passed, so the 60 leagues on HOLD keep their
current entries. `usl-super-league`, which is routed today and now holds at 0/8 clubs, is therefore
not wiped by this run. It changes no routing — see the "reachable is not routed" row above.
...
- **Eight leagues were publishing a final table mid-season; the fix is in the builder, and CI
  ...
  **The verified set is eight, not the eleven first reported:** `sweden-allsvenskan` (16 of 30
  ...
  `scripts/build_league_data.py`); the third is fixed too, in
  `scripts/select_daily_leagues.py` — the daily job now also rebuilds a league that claims to be
  finished but played within the last 30 days, which took the matrix from 28 to 36 jobs and added
  exactly the eight that were stuck. **The guard's yardstick is now the league's own most recent completed season, measured as
  ...
  configuration and would have broken identically at matchday 17. Suite 2237 passed, 39 skipped, 0 failed.
...
- Decide whether `refresh-fast.yml` should pass its `select` step's output to `--refresh-selected`
  instead of re-deriving it. `fast_refresh.main()` calls `select_leagues()` a second time, so the
  selection the workflow logs is not the selection the run acts on. Observed on `32646234681`:
  the log says 9 leagues at 14:42:17, the report covers 8 at 14:42:48, and `liga-mx` left the
  match window in the 31 seconds between them. Nothing was lost — `liga-mx` had been refreshed at
  14:17 — and `refresh_selected` has no silent-skip branch, so a league is always either refreshed
  or reported failed. It is logged here because an unreconcilable count in a log is how both
  2026-08-23 defects stayed hidden, and because the trade-off is real: passing the earlier
  selection makes the log authoritative but acts on a staler window.
